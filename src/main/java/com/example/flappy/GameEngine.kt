...
import com.example.flappybobayt.Bird
...
class GameEngine {
    ...
    fun updateAndDrawBoba(canvas: Canvas) {
        if (gameState == 1 ) {
            if (bird.getY() < (AppConstants.SCREEN_HEIGHT - AppConstants.bitmapBank.getBobaHeight())
                || bird.getVelocity() < 0) {
        ...
        canvas.drawBitmap(AppConstants.bitmapBank.getBoba(currentFrame), bird.getX().toFloat(), bird.getY().toFloat(), null)
